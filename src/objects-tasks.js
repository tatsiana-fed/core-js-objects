...
function group(array, keySelector, valueSelector) {
  return array.reduce((contryMap, item) => {
    const key = keySelector(item);
    const value = valueSelector(item);

    if (!contryMap.has(key)) {
      contryMap.set(key, []);
    }
    contryMap.get(key).push(value);
    return contryMap;
  }, new Map());
}
...
const single = [selectors.element, selectors.id, selectors.pseudoElement];

const cssSelectorBuilder = {
  result: '',
  add(value, order) {
    if (this.order === order && single.includes(order)) {
      throw new Error(
  ...
  element(value) {
    return this.add(value, selectors.element);
  },

  id(value) {
    return this.add(`#${value}`, selectors.id);
  },

  class(value) {
    return this.add(`.${value}`, selectors.class);
  },

  attr(value) {
    return this.add(`[${value}]`, selectors.attribute);
  },

  pseudoClass(value) {
    return this.add(`:${value}`, selectors.pseudoClass);
  },

  pseudoElement(value) {
    return this.add(`::${value}`, selectors.pseudoElement);
  },
